...
| **8 — Platform polish** | Move `prose_telemetry_dashboard.py`, `prose_telemetry_vale_export.py`, `prose_telemetry_corpus.py`, `prose_telemetry_diff.py` from book repo into `apps/`; update book's `Makefile` to invoke `prose` CLI for all editorial functions. Sub-task: migrate the remaining 24 handcount stdlib detectors to the registry, one family-batch at a time. `prose init` CLI now scaffolds book.editorial.yaml for new books. | 2-3d | Book repo `build/` shrinks to book-specific glue; all editorial tasks invoke `prose` CLI; handcount script retires entirely | 🚧 batches 0 (voice — 3) + 1 (classical rhetoric — 4) + 2a (repetition/echo — 4) + 2b (chain — 3) + 3 (pivot/inference — 4) done; 24 remaining |
