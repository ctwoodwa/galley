...
| **8 — Platform polish** | Migrate all handcount stdlib detectors to the registry, one family-batch at a time. `prose init` CLI scaffolds book.editorial.yaml for new books. Dashboard scripts (`prose_telemetry_dashboard.py`, `prose_telemetry_vale_export.py`, `prose_telemetry_corpus.py`, `prose_telemetry_diff.py`) and book Makefile rewrite remain as follow-ups. | 2-3d | Handcount detector count reaches 0; all editorial tasks invoke `prose` CLI | ✅ 42/42 detectors migrated across batches 0–6 (voice 3, classical rhetoric 4, repetition 4, chain 3, pivot/inference 4, density/style 10, grammar/mechanics 8, structural 6). 285 tests green. |
