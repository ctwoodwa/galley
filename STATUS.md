...
| `apps/web/src/api/pluginRegistry.ts` + Services Section auth surface | **Plugin manifest registry** loaded at build time via Vite `import.meta.glob` from `packages/plugins/plugins/*/manifest.json` — typed accessors `getPlugin(id)`, `listPluginsForCapability(capability)`, `authCategoryForProvider(id)`. ServicesSection's per-slot AdvancedDisclosure now branches on the manifest's `authentication.type`: `api-key` plugins render the existing SecretField (header/query-param/bearer aware), `oauth2`/`oidc` plugins render a staged "Sign in with <provider>" button + flow + scopes + MFA hint (button disabled; OAuth runtime ships in Phase 2), `mtls` plugins render a read-only note, `none` hides the auth field. All 8 current cloud plugins resolve to `api-key`; the OAuth surface is forward-compat scaffolding ahead of the first OAuth-protected plugin landing. |
